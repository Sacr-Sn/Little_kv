...
string ManifestManager::searchFromSst(const int& level, const string& key, FdCache& fd_cache) {
    for (const auto &file : file_list) {
        if (file.level != level) continue;
        // key 范围过滤
        if ((key < file.smallest_key) || (key > file.largest_key)) {
            wal.log("[Little_kv::get "+file.filename+"] : key ?????" + key + ", smallest_key:" + file.smallest_key + ", largest_key:" + file.largest_key);
            continue;
        }
        // 布隆过滤器
        ...
        // key可能存在该文件，需要打开文件查询
        wal.log("[ManifestManager::searchFromSst "+key+"] : 正在遍历：" + file.filename);
        ...
        if (fp && fp->is_open()) {
            ...
            wal.log("[ManifestManager::searchFromSst] : ??fd_cache???????");
        }
    }
    return "NOT_FOUND";
}
